...
public class ShopArmor : ShopItem
{
    public GameObject itemPrefab { get; }

    public ShopArmor(string prefabPath, string name, int cost, int minDebtUnlock, int minKillsUnlock) :
        this(prefabPath, name, "", cost, minDebtUnlock, minKillsUnlock)
    { }

    public ShopArmor(string prefabPath, string name, string desc, int cost, int minDebtUnlock, int minKillsUnlock) :
    ...
    protected static GameObject getPrefab(string path)
    {
        ...
        if (itemPrefab == null)
        {
            Debug.LogError("Erreur d'initalisation du ShopArmor pour le Prefab " + path);
        }
    ...
    // Methode appelée lors de l'achat, ici on équipe l'armure au joueur
    override
    public void buyCallback(PlayerController player)
    {
        player.changementArmure(itemPrefab);
    }
